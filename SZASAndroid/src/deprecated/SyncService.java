...
public class SyncService extends Service{
	...
	private static SQLLocalDAO.DBContentProvider getContentProvider(Context context){
		ContentResolver cr = context.getContentResolver();
		ContentProviderClient client = cr.acquireContentProviderClient(SQLLocalDAO.DBContentProvider.AUTHORITY);
		return (SQLLocalDAO.DBContentProvider) client.getLocalContentProvider();
	}
	...
	private static void performSyncImplementation(Context context, Account account, Bundle extras, SyncResult syncResult)
	{
		if(extras.getBoolean(ContentResolver.SYNC_EXTRAS_INITIALIZE, false)){
			if(account!=null && ContentResolver.getIsSyncable(account, SQLLocalDAO.DBContentProvider.AUTHORITY)<0){
				try {
					ContentResolver.setIsSyncable(account, SQLLocalDAO.DBContentProvider.AUTHORITY, getIsSyncable(context, account)? 1 : 0);
